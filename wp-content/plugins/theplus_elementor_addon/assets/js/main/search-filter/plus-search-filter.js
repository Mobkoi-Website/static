	...
	var WidgetSearchFilterHandler = function($scope, $) {
		...
		let seaList = document.querySelectorAll('.'+connId);
			seaList.forEach(function(item, index) {
				...
				option[index]['enable_archive_search'] = (basic.enable_archive) ? 'true' : '';
			});
				...
				if(url && url.search){
					...
					form.forEach(function(self) {
						let seafield = (self && self.dataset && self.dataset.field) ? JSON.parse(self.dataset.field) : '',
							...
							let PriceRange = document.querySelectorAll('.tp-range');
							if( PriceRange.length > 0 ){
								PriceRange.forEach(function(range, idx) { 
									let rangeattr = (range.dataset && range.dataset.sliderattr) ? JSON.parse(range.dataset.sliderattr) : '';
									if(rangeattr){
										let Name = (rangeattr.name) ? rangeattr.name : '',
											field = (rangeattr.field) ? rangeattr.field : '',
											min = (rangeattr.minValue) ? rangeattr.minValue : 0,
											max = (rangeattr.maxValue) ? rangeattr.maxValue : 10000,
											type = (rangeattr.type) ? rangeattr.type : 'taxonomy',
											PriceSymbol = (rangeattr.pricesymbol) ? rangeattr.pricesymbol : '',
											Geturl = (Name) ? params.get(Name + '-' + min + '-' + max) : '';
											
											if(Geturl){
												var selectval = Geturl.split(",");
													
													rangeattr['value'] = selectval
													range.noUiSlider.set( selectval )
													
												if(tagHandle !== null){
													let TagVal = `${PriceSymbol} ${selectval[0]} - ${PriceSymbol} ${selectval[1]}`,
														TagName = FilterTagTitle( field, PriceRange[idx], TagVal);
														
													let priceTag = ('<div class="tp-filter-container"><a class="tp-tag-link" data-type="range" data-name="'+Name+'" data-id="'+Geturl+'"><span class="tp-filter-tag"> <i class="fa fa-times" aria-hidden="true"></i> '+TagName+'</span></a>'),
														...
														Filter_Tags.push(priceTag);
												}

												fieldValue[field] = rangeattr;
											}										
									}
								});
							}
		...
		let WooBtnTmp = '',
			Tempbtn = true;
		let woobutton = document.querySelectorAll('.tp-woo-button .tp-buttonBox input[type="radio"]');
			if(woobutton.length > 0){
				woobutton.forEach(function(self) {
					self.addEventListener("click", function(el){
						if(WooBtnTmp == self && Tempbtn){
							this.checked = false;
							$(this).trigger("change");
							Tempbtn = false;
						}else{
								...
								let name = (rangeattr && rangeattr.field) ? rangeattr.field : '',
									...
									if(values){
										...
										if(basic && basic.URLParameter){
                                            ...
                                            let urlname = rangeattr.name + '-' + parseInt(rangeattr.minValue) + '-' + parseInt(rangeattr.maxValue);

											urlHandler(urlname, Urldata)
										}
		...
		var TabbingHandle = function(GetHtml,index){
			let tabTick_wrap = (GetHtml && GetHtml[index]) ? GetHtml[index].querySelectorAll('.tp-tabbing-wrapper') : '';
